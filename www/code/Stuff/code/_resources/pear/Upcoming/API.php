<?php
	...
	class Upcoming_API {
		...
		var $secure_methods = array(
			'event.add', 'event.edit', 'event.addTags', 'event.removeTag', 'venue.add', 'venue.edit',
			'watchlist.add', 'watchlist.remove', 'group.add',	'group.edit', 'group.join', 'group.leave',
			'group.addEventTo', 'group.admin.removeEvent'
		);
		...
		function Upcoming_API($params = array()){
		...
		function callMethod($method, $params = array()){
			...
			if ($this->_cfg['api_token']){

				$p['token'] = $this->cfg['api_token'];
			}
			...
			$request_type = HTTP_REQUEST_METHOD_GET;
			$endpoint = $this->_cfg['endpoint'];
						
			$is_secure_method = $this->isSecureMethod($method);
			
			if ($is_secure_method) {
				$request_type = HTTP_REQUEST_METHOD_POST;
			}
			else {
				$endpoint .= '?' . $body;
			}

			$req =& new HTTP_Request($endpoint, array('timeout' => $this->_cfg['conn_timeout']));
		...
		function isSecureMethod($method){
			return in_array($method, $this->secure_methods);
		}
